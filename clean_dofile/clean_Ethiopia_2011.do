...
collapse (max)hh_s5bq02, by(household_id  ea_id hh_s5bq00)
label var hh_s5bq02 "# of Days specific food is eaten"

replace hh_s5bq02 = 0 if hh_s5bq02==.
...
save ethiopia_2011.dta, replace
...
collapse (max)hh_s5bq02, by(household_id hh_s5bq00)
label var hh_s5bq02 "# of Days specific food is eaten"
replace hh_s5bq02=0 if hh_s5bq02==.
...
collapse (sum) HDDS, by(household_id)
...
merge m:m household_id using ethiopia_2011
drop _merge
sort household_id 
...
tab  hh_s7q06
*Describing the data
foreach var of varlist hh_s7q07_m-hh_s7q07_k{
	tab `var'
}
...
egen H_Count=rowtotal (hh_s7q07_m_num-hh_s7q07_k_num)
...
use "sect_cover_hh_w1",clear

 
keep household_id ea_id rural  saq03 saq04 pw saq01 hh_saq09
...
* Merge in geolocation 
 use Pub_ETH_HouseholdGeovariables_Y1.dta,clear
keep household_id LAT_DD_MOD  LON_DD_MOD srtm srtm_5_15 sq1 sq2 dist_road dist_admctr  dist_popcenter fsrad3_agpct
...
label variable FS_month "Month FS module was administered"
 ...
 keep household_id FS_month FS_year 
...
egen number_celphones =sum(hh_s10q01 ) if hh_s10q00  ==8,by(household_id)

egen Refrigerator = sum(hh_s10q01 ) if hh_s10q00  ==22,by(household_id)
replace Refrigerator =1 if Refrigerator !=0 & Refrigerator !=.


egen Radio = sum(hh_s10q01 ) if hh_s10q00  ==9,by(household_id)
replace Radio =1 if Radio !=0 & Radio !=.


egen Television = sum(hh_s10q01 ) if hh_s10q00  ==10,by(household_id)
replace Television =1 if Television !=0 & Television !=.


egen Bicycle = sum(hh_s10q01 ) if hh_s10q00  ==14,by(household_id)
replace Bicycle =1 if Bicycle !=0 & Bicycle !=.


egen Motorcycle = sum(hh_s10q01 ) if hh_s10q00  ==15,by(household_id)
replace Motorcycle =1 if Motorcycle !=0 & Motorcycle !=.


egen Car = sum(hh_s10q01 ) if hh_s10q00  ==23,by(household_id)
replace Car =1 if Car !=0 & Car !=.

egen r1 = sum(Refrigerator),by(household_id)
egen r2 = sum(Radio),by(household_id)
egen T = sum(Television),by(household_id)
egen B = sum(Bicycle),by(household_id)
egen M = sum(Motorcycle),by(household_id)
egen C = sum(Car),by(household_id)


duplicates drop household_id,force

keep household_id r1 r2 T B M C cell_phone number_celphones
...
use "sect9_hh_w1",clear

keep household_id hh_s9q06  hh_s9q07  
...
keep household_id   roof_natural roof_iron roof_other floor_cement floor_tile floor_dirt
...
*save "/Users/yujunzhou/Box Sync/lsms/FCS_ethiopia_2011.dta",replace
drop if FCS ==0 
drop if HDDS ==0

save "C:\Users\Administrator\Desktop\lsms\cleaned_dataset\FCS_2011_Ethipopia.dta", replace
