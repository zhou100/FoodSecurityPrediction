...
ggsave("RCSI_2013_full.png", plot = last_plot(),device = "png",path = "output/figures/",
       dpi = 2000, limitsize = TRUE)
...
ggsave("RCSI_2013_ipc_vs_actual.png", plot = last_plot(),device = "png",path = "output/figures/",
       dpi = 2000, limitsize = TRUE)
...
ggsave("HDDS_2013_full.png", plot = last_plot(),device = "png",path = "output/figures/",
       dpi = 2000, limitsize = TRUE)
...
ggsave("HDDS_2013_ipc_vs_actual.png", plot = last_plot(),device = "png",path = "output/figures/",
       dpi = 2000, limitsize = TRUE)
# 
# ggplot(as.data.frame(plot_long),aes(x=HDDS, color=level)) + geom_density(alpha=0.25,show.legend = TRUE) +xlim(1.5, 7.6) + geom_vline(xintercept=3,linetype=2) + geom_vline(xintercept=6,linetype=2)
...
rcsi_poor<-read.csv("data/rcsi_poor.csv")

measure_only<-malawi[,c('case_id','FCS','RCSI','HDDS')]
poor_rcsi<- measure_only[measure_only$RCSI>40,]
